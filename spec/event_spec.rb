...
describe TreasureData::Logger::Event do
  context 'preset' do
    ...
    it 'action' do
      test_logger.should_receive(:post).with(:doit, {:foo=>:bar, :uid=>"uid1"}).twice
    ...
    it 'register' do
      test_logger.should_receive(:post).with(:register, {:uid=>"uid1"}).twice
    ...
    it 'login' do
      test_logger.should_receive(:post).with(:login, {:uid=>"uid1"}).twice
      TD.event.login("uid1")
      TD.event.attribute[:uid] = "uid1"
      TD.event.login
    end
    ...
    it 'pay' do
      test_logger.should_receive(:post).with(:pay, {:category=>"cat", :sub_category=>"subcat", :name=>"name", :price=>1980, :count=>1, :uid=>"uid1"}).twice
